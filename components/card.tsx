    ...
    return (
      <div key={`key-${job.id}`} className="job-card py-5 px-4 bg-header-bg-color rounded-lg  transition duration-[0.2s]  transform hover:scale-[1.02] ">
        ...
        <div className="job-card-buttons flex items-center justify-between w-full mt-1 gap-4">
          ...
          <Link
            href={`/jobs/${job.id}`}
            className="search-buttons card-buttons-msg  border-none text-button-color inline-block text-center bg-active-color p-[10px] w-full  rounded-md text-xs cursor-pointer font-medium mt-[14px]">
            Overview
          </Link>
        </div>
      </div>
